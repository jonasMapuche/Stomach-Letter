...
namespace Letter.Repositories.SQLites
{
    public class AuxiliarRepository : IAuxiliarRepository
    {
        ...
        public async Task<int> DropTable()
        {
            try
            {
                if (this._error_off) throw new InvalidOperationException("Operation drop table \"Auxiliar\" repository failed!");

                return await this._database.DropTableAsync<Auxiliares>();
            }
            catch (Exception ex)
            {
                this.error_message = ex.Message;
