...
namespace Letter.Repositories.SQLites
{
    public class ArtigoRepository : IArtigoRepository
    {
        ...
        public async Task<int> DropTable()
        {
            try
            {
                if (this._error_off) throw new InvalidOperationException("Operation drop table \"Artigo\" repository failed!");

                return await this._database.DropTableAsync<Artigos>();
            }
            catch (Exception ex)
            {
                this.error_message = ex.Message;
