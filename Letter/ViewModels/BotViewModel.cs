...
namespace Letter.ViewModels
{
    ...
    public partial class BotViewModel : ObservableObject, IQueryAttributable
    {
        ...
        private Dictionary<string, string> _open;
        ...
        public BotViewModel(HttpService httpService, MessageService messageService, PerceptionService perceptionService, GrammarService grammarService)
        {
            try
            {
                ...
                this._kind_bot = this._settingService.KindBot;
        ...
        private async Task<List<Mechanism>> BotDecision(string language, List<Recite> recites)
        {
            try
            {
                ...
                if (predicatives.Count == 1)
                {
                    ...
                    predicative_adjective = predicative_talks.FindAll(index => index.Etiology.Contains(this._adjective));
        ...
        private async Task<List<Mechanism>> CommandButton(string language, List<Talk> verbs, List<Talk> nouns, List<Talk> adjectives, List<Recite> explanatories)
        {
            try
            {
                ...
                verbs_speak = this._speak.Where(index => index.Value.Contains(language))
                    .ToDictionary(index => index.Key, index => index.Value).Keys.ToHashSet();
                HashSet<string> verbs_preview = new HashSet<string>();
                verbs_preview = this._preview.Where(index => index.Value.Contains(language))
                    .ToDictionary(index => index.Key, index => index.Value).Keys.ToHashSet();
                ...
                if ((FindPredication(verbs, verbs_preview)) && (FindOwner(nouns, nouns_camera)))
                {
                    string result = string.Empty;
                    result = await PreviewCamera(language);
                    mechanisms = MountMechanism(result, Tacit.Copy);
                    return mechanisms;
                }
        ...
        private async Task<string> RotateCamera(string language, string kind)
        {
            try
            {
                ...
                {
                    this.CameraPreview?.SetRotate = Rotate.Front;
                    response = $"{camera.ToArray()[0]} {rotate.ToArray()[0]} {front.ToArray()[0]}.";
                }
                if (kind == rear.ToArray()[0])
                {
                    this.CameraPreview?.SetRotate = Rotate.Rear;
                    response = $"{camera.ToArray()[0]} {rotate.ToArray()[0]} {rear.ToArray()[0]}.";
                }
                return response;
            }
            catch (Exception ex)
        ...
        private async Task<string> FlashCamera(string language, string kind)
        {
            try
            {
                ...
                {
                    this.CameraPreview?.SetFlash = Flash.On;
                    response = $"{flash.ToArray()[0]} {turn_on.ToArray()[0]}.";
                }
                if (kind == off.ToArray()[0])
                {
                    this.CameraPreview?.SetFlash = Flash.Off;
                    response = $"{flash.ToArray()[0]} {turn.ToArray()[0]} {off.ToArray()[0]}.";
                }
                if (kind == auto.ToArray()[0])
                {
                    this.CameraPreview?.SetFlash = Flash.Auto;
                    response = $"{flash.ToArray()[0]} {turn.ToArray()[0]} {auto.ToArray()[0]}.";
                }
                return response;
            }
            catch (Exception ex)
        ...
        private async Task<string> PreviewCamera(string language)
        {
            try
            {
                if (this._error_off) throw new InvalidOperationException("Operation preview camera \"Bot\" view model failed!");
                ...
                HashSet<string> camera = this._camera
                    .Where(index => index.Value.Contains(language))
                    .ToDictionary(index => index.Key, index => index.Value).Keys.ToHashSet();
                HashSet<string> open = this._open
                    .Where(index => index.Value.Contains(language))
                    .ToDictionary(index => index.Key, index => index.Value).Keys.ToHashSet();

                string response = string.Empty;
                if (this.CameraPreview != null)
                    response = $"{camera.ToArray()[0]} {preview.ToArray()[0]} {open.ToArray()[0]}.";
                else
                {
                    WeakReferenceMessenger.Default.Send(new NoticeService("preview"));
                    response = $"{camera.ToArray()[0]} {preview.ToArray()[0]}.";
                }
                return response;
            }
            catch (Exception ex)
        ...
        private async Task<string> StopCamera(string language)
        {
            try
            {
                ...
                if (this.CameraPreview != null)
                {
                    WeakReferenceMessenger.Default.Send(new NoticeService("stop"));
        ...
        private void OnImageBytes(object? sender, EventArgs e)
        {
            try
            {
                if (this._error_off) throw new InvalidOperationException("Operation on image bytes \"Bot\" view model failed!");
